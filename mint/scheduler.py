...
ActionType = Literal["execute", "delay", "cancel", "replace"]
...
def schedule_intents(
    ...
    for intent in intents:
        ...
        if call_prob <= 0.0 and scheduler_cfg.get("enable_cancel", True):
            non_candidates.append(WarmupAction("cancel", intent, gain, "function_not_expected_to_be_called"))
        elif hot_until > 0.0 and hot_until - now_sec >= retention * 0.5 and scheduler_cfg.get("enable_cancel", True):
            non_candidates.append(WarmupAction("cancel", intent, gain, "function_already_hot_with_sufficient_retention"))
            ...
            non_candidates.append(WarmupAction("cancel", intent, gain, "negative_or_below_threshold_gain"))
        else:
